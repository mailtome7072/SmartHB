...
### Sprint 19: 그리드 정렬/스크롤 통일 + 인쇄 캘린더 고도화 + 수업관리 표시 규칙 정비 + 대시보드 레이아웃 + 학년 자동승급 + 학교급 필터링 + 인쇄 아키텍처 전면 재설계 (2주) ✅ 완료 (2026-07-08)

> 계획 문서: `docs/sprint/sprint19.md`
> 실사용 원장님 UI/UX 피드백 8건 + 추가 요구 2건(학년 자동승급, 학교급 필터링) 반영. V310 마이그레이션 1건.
> 계획 완료 후 실사용 중 발견된 다수의 후속 버그 수정 및 추가 기능(수강생대장 출력, 인쇄 팝업 아키텍처 전환 등) 반영.
> develop 직접 머지 (단일 개발자 정책, 2026-07-08)

#### 작업 목록

- ✅ **T0**: Sprint 18 회고 액션 (A113 상수 쌍 목록화, A116 인쇄 캘린더 동적 행 수)
- ✅ **T1**: 공통 정렬 인프라 + 원생 목록 기본 정렬 학년+이름순 통일
- ✅ **T2**: 출결 그리드 컬럼 정렬 + 이중 스크롤 컨테이너 해소 (가로 스크롤 접근성)
- ✅ **T3**: 청구 그리드 컬럼 정렬 + sticky header + 스크롤 개선
- ✅ **T4**: 교습일정 인쇄 캘린더 — 교습일 Red 테두리 + 기간성 일정 밴드 오버레이 + 폰트 확대
- ✅ **T5**: 수업관리 주보기 — 화살표 제거, 이름만 표시, 2xN 규칙 위반 버그 수정
- ✅ **T6**: 수업관리 일보기 10xN 규칙 + 월/주/일보기 캘린더 라인 진하게
- ✅ **T7**: 대시보드 당일수업+생일 위젯 상하 배치 (높이 2:1)
- ✅ **T8**: 학년 자동 승급 — 매년 1월 이후 최초 실행 시 확인 다이얼로그 + 일괄 UPDATE
- ✅ **T9**: 학교급 기반 학교 선택 필터링 — V310 마이그레이션 + codes UI + student-form 필터
- ✅ **T10**: 통합 검증

#### 완료 기준 (Definition of Done)
- ✅ 모든 원생 그리드 기본 정렬 학년+이름순 + 컬럼 클릭 정렬 동작
- ✅ 출결/청구 그리드 가로 스크롤 즉시 접근 가능
- ✅ 교습일정 인쇄: Red 테두리 + 밴드 오버레이 + 폰트 확대 + 동적 행 수
- ✅ 주보기 2xN 규칙 준수 + 일보기 10xN 규칙
- ✅ 대시보드 상하 배치 2:1 비율
- ✅ 학년 자동 승급(1월 최초 실행 확인 다이얼로그) + 학교급 기반 학교 필터링
- ✅ V310 마이그레이션 정상 적용 + 사용자 수동 검증 완료 (2026-07-08)

---

### Sprint 20: 교습기간 다월 걸침 결함 일괄 수정 — 청구 규칙 전환 + 삭제 기능 + 인쇄/출결 버그 + 실 DB 보정 (2주) ✅ 완료 (2026-07-19)
...
> develop 직접 머지 (단일 개발자 정책, 2026-07-19)

#### 작업 목록

- ✅ **T1**: 청구 대상 규칙 교습기간 기준 전환 — `generate_bills_impl`의 period를 `year_month_range`(달력월) 대신 `study_periods.start_date/end_date`로 교체. study_periods 미등록 월 차단. `compute_mid_month_flag` 교습기간 경계 사용. `effective_from` 필터 추가. **`get_billing_summary_impl` 동기화(유령 버튼 방지)**. 회귀 방지 단위 테스트 6건
- ✅ **T2**: 청구 삭제 가드 정책 ADR (B안 확정: 미수납이면 draft/confirmed 삭제, 수납완료 거부) — ADR-010 작성 완료
- ✅ **T3**: 청구 삭제 백엔드 — `delete_bill` 커맨드 + `BillDeleted` audit variant + lib.rs 등록 + PRAGMA FK 검증 + 단위 테스트 4건
- ✅ **T4**: 청구 삭제 프론트엔드 — IPC 래퍼 + 삭제 버튼(미수납만 활성) + 확인 다이얼로그 + TanStack Query 무효화
- ✅ **T5**: 실 DB 보정 절차 문서 (`docs/sprint/sprint20/data-correction-procedure.md`) / **T5-b**: 퇴교 취소 → 월별 데이터 재생성 운영 워크플로우 안내
- ✅ **T6**: 교습일정 인쇄 3개 월 이상 걸침 달력 깨짐 수정 — **주 월 달력 한 장** 표기(앞뒤 이웃 달 며칠은 교습일로 강조, 그리드 밖 기간만 멀티페이지 폴백)
- ✅ **T7-A**: 출결 버그 A 수정 — 부분생성 오판(생성 버튼 숨김) 수정. `count_ungenerated`를 기대 수업일 대비 판정으로 교체. 회귀 테스트 1건
- ⏸️ **T7-B (R136)**: 출결 버그 B(다월 그리드 표시/태깅 불일치) — 그리드 컬럼 재설계 범위가 Sprint 20 용량 초과. **후속 스프린트로 분리** (Sprint 21 후보)
- ✅ **T8**: 통합 검증 — cargo test 441 passed / clippy --all-targets clean / cargo check --features cipher / pnpm lint / tsc clean / pnpm build 전수 통과
...
- ✅ 교습일정 인쇄 3개월+ 걸침 정상 출력 (주 월 단일 달력 방식)
- ✅ 출결 버그 A 수정 — 부분생성 상태에서 재생성 경로 열림
- ⏸️ 출결 버그 B — 후속 Sprint 21로 분리 (R136)
...
### Sprint 22: 보강 분 단위 부분 차감 전환 + 출결 그리드 z-index 수정 (2주) ✅ 완료 (2026-07-21)

> 계획 문서: `docs/sprint/sprint22.md`
> 보강 매칭 모델을 일 단위(PI-02 옵션 A)에서 분 단위 부분 차감으로 전환. 유실 데이터 자동 백필. 출결 그리드 sticky z-index 수정.
> DB 마이그레이션 V311(스키마 변경) + V312(백필) 완료 / 새 의존성 없음

#### 작업 목록

- ✅ **T0**: 부분 보강 스키마 설계 ADR (brainstorming) — `docs/arch/adr-011-partial-makeup-schema.md`
- ✅ **T1**: DB 마이그레이션 V311 — 부분 보강 스키마 변경
- ✅ **T2**: 보강 등록 로직 부분 차감 전환 (`makeup.rs` 핵심)
- ✅ **T3**: 보강 취소 부분 차감 대응 (`makeup.rs`)
- ✅ **T4**: 보강 대상 목록/요약/소멸/진단 쿼리 일괄 변경 (8개 파일 회귀 체크리스트)
- ✅ **T5**: 유실 데이터 자동 백필 마이그레이션 V312 + 멱등 설계 + 마이그레이션 직전 사전 스냅샷 백업
- ✅ **T6**: 백필 결과 안내 UI — 완전 자동·무알림 확정 (T6 별도 UI 불필요)
- ✅ **T7**: 보강 등록 UI 잔여분 기반 전환 (`MakeupRegisterDialog`) + A126 prop 명확화
- ✅ **T8**: 출결 그리드 sticky z-index 수정 (`AttendanceGrid`)
- ✅ **T9**: 통합 검증 (cargo test 457 passed + clippy + cipher check + lint + tsc + build 전수 통과)

#### 완료 기준 (Definition of Done)
- ✅ ADR-011 부분 보강 스키마 설계 결정 완료
- ✅ V311/V312 마이그레이션 정상 적용 + 백필 멱등성 확인
- ✅ 부분 보강 비즈니스 규칙 단위 테스트 100% 커버
- ✅ 회귀 위험 체크리스트 8항목 전수 변경 + 테스트 통과
- ✅ 출결 그리드 z-index 수정 (시각 검증은 배포 단계)
...
> 근본 원인: 라이브 SQLite DB를 클라우드 동기화 폴더(MYBOX)에 열어둔 채 사용 — 빈 DB 날조 + 커넥션 키 유실 + 복원 미발동
> DB 마이그레이션 없음 (V312 유지) / 새 의존성 없음
...
- ⏸️ **A127**: cancel_makeup N+1 배치 전환 — makeup.rs 미수정으로 범위 밖
...
### 기능 요청 — 선(先)수업 / 미리 수업 (원장, 2026-08-01)
> **배경**: 원생 사정으로 **미래 정규 수업일 X에 못 나올** 때, X 이전의 다른 날에 그 수업을 **미리** 진행하고 싶다.
> **현재 한계**: 대상 날에 수업이 없으면 기존 "수업일 이동"으로 처리 가능. 그러나 **대상 날에 이미 정규 수업이 있으면 그날 수업을 하나 더 추가할 방법이 없다** ← 구현 필요.
- ⬜ 이미 수업이 있는 날에 "추가 수업(선수업)" 1건을 더 등록하는 UI/IPC (`makeup_attendances`는 동일 일자 중복 허용 — 데이터 모델은 이미 지원, PRD §6.2)
- ⬜ 미래 결석 예정일 X와의 연결 (선수업 완료 표시 → X를 결석/미보강으로 집계하지 않음)
- **설계 시 결정 필요**: (a) 기존 보강(사후) 도메인과 통합 vs 별도 "선보강" 개념, (b) 청구 시수 계산 반영, (c) 보강 소멸 로직과의 관계, (d) 다월 교습기간 year_month 태깅 규칙 준수(Sprint 24 함정), (e) 대상 날 수업 없음(이동) vs 있음(추가)의 UI 일원화 여부

### ADR-012 B안: 로컬 라이브 DB + 클라우드 핸드오프 (다음 Phase 후보)
> **승격 트리거**: Sprint 23(v1.5) 배포 후에도 **클라우드 간섭에 의한 손상/복원 이벤트**가 관찰되면, `phase-planner`로 B안 설계에 착수한다.
> ADR-012 A안(클라우드 유지+접근 강화)의 잔여 리스크(활성 사용 중 torn-sync)가 실사용에서 재현될 경우의 에스컬레이션 경로.
> 범위: 라이브 DB를 OS 로컬(`app_data_dir`)로 이전 + 클라우드는 스냅샷 핸드오프 전용 + 일회성 이전 마이그레이션. 핸드오프 서브시스템 신규 구축 필요 (Phase 규모).
- ⬜ phase-planner 설계 (승격 조건 충족 시)
