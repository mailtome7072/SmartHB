...
| 현재 Phase | **Post-v1.3** — Sprint 22 🔄 진행 중 |
| 다음 마일스톤 | Sprint 22 완료 → 보강 부분 차감 배포 |
...
- ⬜ **T5**: 유실 데이터 자동 백필 마이그레이션 V312 + 멱등 설계
- ⬜ **T6**: 백필 결과 안내 UI (토스트/다이얼로그)
- ⬜ **T7**: 보강 등록 UI 1시간 단위 선택 전환 (`MakeupRegisterDialog`)
...
| M14: 보강 부분 차감 | Post-v1.3 | Sprint 22 | +44주 | 보강 분 단위 부분 차감 + 유실 데이터 백필 + 출결 그리드 z-index 수정 |
