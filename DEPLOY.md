...
## 2026-07-22 | v1.4.0 | Sprint 22 — 보강 분 단위 부분 차감 전환 + 출결 그리드 z-index 수정

브랜치: `develop → master` 직접 머지 (단일 개발자 정책)

### 스테이징 검증 (develop 로컬)
- ✅ sprint-review 에이전트 실행 (코드 리뷰 + 자동 검증, Critical/High 0건)
- ✅ pnpm tauri:dev 실행하여 앱 동작 수동 확인
- ✅ sqlx migrate run — V311/V312 마이그레이션 정상 적용 확인 (개발 DB 기준)
- ✅ 보강 등록 시 잔여분 UI 정상 표시 확인 (2h 결석 → 1h 보강 후 잔여 1h 재노출 정상)
- ✅ 출결 그리드 상하 스크롤 시 헤더 고정 시각 확인 (z-index 수정 확인)
- ✅ 보강 취소 후 결석 잔여분 원복 확인
- ✅ 추가 UX 개선 구현·검수 완료 (커밋 7b85573/3b9e05d/cec27f7/dea3601)
- ⬜ 클라우드 동기화 폴더 cipher-on 실 DB 백필 스모크 — 원장님 PC 인스톨러 설치 후 확인으로 대체

### 프로덕션 배포 (master 머지 + v태그 push)
- ⬜ develop → master 직접 머지
- ⬜ v1.4.0 태그 push → GitHub Actions 빌드 완료
- ⬜ GitHub Release 아티팩트 업로드 확인
  - ⬜ Windows: SmartHB_1.4.0_x64-setup.exe
  - ⬜ macOS: SmartHB_1.4.0_aarch64.dmg

### CV — 아티팩트 검증
- ⬜ gh release view v1.4.0 으로 Release 확인
- ⬜ 다운로드 URL 유효성 확인
- ⬜ 원장님 PC 인스톨러 설치 — V311/V312 백필 자동 실행 확인

이전 배포 기록: `docs/deploy-history/2026-07-22.md` (Sprint 22 스테이징 아카이빙)
