...
### Hotfix: sprint7-post-review-cleanup (2026-05-23)

버전: v0.3.2
브랜치: `hotfix/sprint7-post-review-cleanup` → `master` 직접 머지 (단일 개발자 정책)

#### 변경 내역
- R50: `NEXT_PUBLIC_DEV_AUTOLOGIN` 보안 경고 주석 강화 (LockScreen.tsx, LockWarning.tsx, .env.example)
- I-S2-6: `load_salt_backs_up_corrupted_file` 테스트 `#[ignore]` 가드 (auth.rs)
- 코드 리뷰 Medium 이슈 보완: LockWarning.tsx R50 주석 누락 수정

#### 자동 검증 완료 항목
- ✅ cargo test --lib: 185 passed, 1 ignored (I-S2-6 정상 가드) — Flaky 1건(`release_lock_atomic_removes_self_owned_lock`, 단독 실행 시 통과, 이번 변경과 무관)
- ✅ cargo clippy -- -D warnings: clean
- ✅ pnpm tsc --noEmit: clean
- ✅ pnpm lint: No ESLint warnings or errors

#### 수동 검증 필요 항목
- ⬜ pnpm tauri:dev 실행 후 자동 로그인(NEXT_PUBLIC_DEV_AUTOLOGIN) 동작 확인
- ⬜ release 빌드 시 env 정리 후 빌드 → 번들에 dev 비밀번호 미포함 확인 (선택)
- ⬜ v0.3.2 GitHub Actions 빌드 완료 확인
- ⬜ GitHub Release v0.3.2 아티팩트 업로드 확인 (Windows .msi/.exe, macOS .dmg)
