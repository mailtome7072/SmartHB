...
- Sprint 24: **수업일 이동 팝업 달력을 교습기간 범위로 확장 (수동검증 발견, B1 프론트 보완)** — 백엔드는 "같은 교습기간 내 이동"을 허용(B1)하는데 팝업 달력이 출발일 달력월 1개월만 그려, 다월 교습기간(예: 8월 7/30~9/2)의 9/1·9/2로 이동할 수 없었다. 달력을 `periodStart~periodEnd` 범위(두 달에 걸침 가능)로 렌더링하고 교습기간 밖 날짜는 비활성. 교습기간 밖(예: 9/30)은 보강으로 안내
- Sprint 24: **청구 주당시간 산정을 "교습기간에 유효했던 스케줄" 기준으로 수정 (수동검증 발견)** — 이전에는 현행 스케줄(`effective_to IS NULL`)만 참조해, 교습기간이 스케줄 변경 적용일보다 앞선 경우(예: 목요일 변경 적용일 7/30 → 7월 교습기간 7/1~7/29) 변경 전 시수를 놓쳐 오산했다(7월이 3h가 아닌 2h로 계산). `generate_bills`·`get_billing_summary` 모두 `effective_from <= period_end < effective_to`(이력 인식)로 교체. 부수 효과로, 7월 "추가 청구 데이터 생성" 유령 버튼(대상엔 잡히나 요금 없는 시수라 생성 스킵 → 클릭해도 무동작)도 해소
...
### Added
- Sprint 24: **시수 변경 시 청구 재확인 팝업 (수동검증 요청)** — 이미 청구가 생성된 교습월의 원생 스케줄(주당 시수)을 변경하면, 해당 청구가 자동 반영되지 않으므로 "청구 관리에서 직접 확인·수정" 팝업을 표시. 신규 IPC `list_affected_bill_months`(변경일 이후 청구 존재 교습월 조회) + 스케줄 편집기 모달. 영향 청구가 없으면 팝업 미표시
- Sprint 24: **다월 교습기간 회귀 테스트 14건 신규** — 경계일(교습기간 시작/종료일이 달력월과 어긋나는 날짜) 시나리오 단위 테스트 일괄 추가 (attendance 9건 + makeup 3건 + diagnosis 2건). A114(`sync_single_date` 이력 패턴 통일) 최종 해소
...
- Sprint 22: **마이그레이션 직전 사전 스냅샷 백업** — 미적용 마이그레이션이 있는 기존 DB는 V311/V312 적용 직전 exit 백업 계층(`backup/exit/`)에 스냅샷을 1회 자동 생성하는 안전장치 (ADR-011 R140)
- Sprint 22: **보강 관리 접근성 UX** — 출결 관리 상단 '보강 관리' 버튼(→수업 관리 보강 탭 바로가기), 보강 관리 목록 원생 이름 클릭 시 결석 이력 다이얼로그, 출결 그리드·보강 목록 원생 이름 표시 통일(상시 링크 + '결석 이력 보기' 힌트)
- Sprint 22: **원생 관리 재원 상태 필터 3분화** — '재원중/전체/퇴원' 택1 세그먼트 필터(백엔드 `withdrawn_only`) + 퇴교생 행에 퇴교일 `(퇴교 YYYY-MM-DD)` 표시
...
- Sprint 22: **결석 이력 부분보강 상태 표시** — `absent`이면서 배분분 > 0인 결석을 '부분보강'으로 구분하고 보강분·잔여 시간 표시 (기존 '미처리' 단일 표기 개선)
