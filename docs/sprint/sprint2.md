...
- **R6** (salt Keychain 저장): **Sprint 2 에서 해소 불가** — DB 안에 salt 보관은 chicken-and-egg. Sprint 3 마법사 통합 시 `{data_root}/salt.bin` 평문 파일로 이전 (T2 이연)
...
- ❌ **T2: R6 salt 저장 위치 이전 — Sprint 3 마법사 통합 시점으로 이연** (0.5일 회수)
  - **이유**: salt 를 DB(`app_settings`) 에 저장하는 안은 chicken-and-egg 충돌 — DB 를 열려면 PRAGMA key 가 필요하고, key 유도에 salt 가 필요. salt 는 평문(DB 외부) 보관이 필수.
  - **재정의된 해소 조건**: Sprint 3 마법사 도입 시 사용자가 지정한 클라우드 동기화 폴더의 평문 파일 `{data_root}/salt.bin` 로 이전. 본 sprint 에서는 Keychain 방식 유지.
  - **회수된 0.5일**: T14 비즈니스 규칙 단위 테스트 보강에 재투자.
  - risk-register R6 갱신: 해소 조건을 "Sprint 3 마법사 통합" 으로 명시.
...
- ❌ **T8: .sqlx 오프라인 캐시 갱신 — 매크로 도입 시점으로 이연** (0.5일 회수)
  - **이유**: 현재 코드에 `query!()` / `query_as!()` 매크로 사용 0건 (Sprint 1 전체가 동적 query + bind 패턴). prepare 가 출력할 게 없음. `SQLX_OFFLINE=true cargo build` 가 이미 통과.
  - **재정의**: T9 IPC 도 동적 query + bind 패턴 유지 (backend.md 의 raw concat 금지 + bind 사용 정책 충족). 매크로 전환은 별도 sprint/backlog.
  - **회수된 0.5일**: T14 비즈니스 규칙 단위 테스트 보강 + simplify 작업에 재투자.
...
| 실작업 가능 시간/일 | 4시간 |
| 총 가용 시간 | 40시간 |
| 총 Task 수 | 12개 (T2/T8 이연) |
| 예상 총 소요 | ~37시간 (T2/T8 각 0.5일 회수) |
| 여유율 | ~7.5% (긴급 이슈 대응용) |
...
**필수**
- ✅ V101~V105 마이그레이션 정상 적용 (V100 폐기, V104 standard_fees 재설계, V105 schools 보강)
- ❌ `.sqlx/` 오프라인 캐시 갱신 — query!() 매크로 미사용으로 이연 (별도 backlog)
- ✅ 루트 라우팅: 첫 실행 시 잠금 화면 자동 진입 (PRD §5.6) — T1
- ✅ IPC 커맨드별 단위 테스트 통과 (인메모리 DB) — 97 tests passed
- ✅ 비즈니스 규칙 단위 테스트 100% 커버 (주 총 수업시간, PI-05 자동 채번 시나리오, 스케줄 이력, 교습비 매칭, UNIQUE 위반 한국어)
- ⚠️ 원생 50명 기준 CRUD 응답 300ms 이내 — cipher off 인메모리만 검증, cipher on + 50명 시드 사용자 환경 실측은 사용자 일정 시
- ❌ R6 salt 이전 — Sprint 3 마법사 통합으로 이연 (chicken-and-egg)
- ✅ R7 release_lock advisory lock 적용 — T3 (release_lock_atomic)
- ✅ R8 cipher on 성능 측정 코드 추가 — T4 (StartupResult timing breakdown + eprintln)
- ✅ `cargo test --manifest-path src-tauri/Cargo.toml` 전체 통과 (97 passed)
- ✅ `cargo clippy --manifest-path src-tauri/Cargo.toml --all-targets -- -D warnings` 통과
- ✅ `pnpm lint` + `pnpm tsc --noEmit` + `pnpm build` 통과 (3 routes prerendered)
