...
| T3: 청구 상태 머신 IPC | 3h | confirm + close + reopen 검증 + 단위 테스트 |
...
### T6: TypeScript IPC 래퍼 + 도메인 타입 (1.5h)
...
### T7: 청구 관리 UI (4h) -- skill: frontend-design
...
### T8: 수납 관리 UI (3h) -- skill: frontend-design
...
- ⬜ **자동 검증 7항목**
  1. `cargo test --manifest-path src-tauri/Cargo.toml` 전체 통과
  2. `cargo clippy --manifest-path src-tauri/Cargo.toml -- -D warnings` clean
  3. `pnpm lint` 통과
  4. `pnpm tsc --noEmit` 통과
  5. `pnpm build` (static export) 성공
  6. `sqlx prepare --manifest-path src-tauri/Cargo.toml` `.sqlx/` 최신
  7. 마이그레이션 self-check (A39): V109와 실제 migrations 1:1 일치
- ⬜ **AC 마킹** -- 아래 AC 전수 검증 후 체크
  - AC-4.9-1: 청구액 = 표준 교습비 매핑 기본값
  - AC-4.9-2: 월 중 입퇴교 시각 구분
  - AC-4.9-3: 확정 후 수정 시 확인 다이얼로그
  - AC-4.9-4: 카드 계열 시 카드사 필수
  - AC-4.9-5: 미확정 청구 상단 배너
  - AC-4.9-6: 일괄 처리 최소 20행
  - AC-4.9-7: 마감 전제조건 (모두 확정)
  - AC-4.9-8: 마감 후 수정 사유 필수
...
- ⬜ 마감 전제조건: 해당 월 모든 청구 confirmed (AC-4.9-7)
- ⬜ 월 중 입퇴교 시각 구분 표시 (AC-4.9-2)
- ⬜ 카드 계열 시 카드사 필수 입력 (AC-4.9-4)
- ⬜ 청구 50명 생성 3초 이내 (PRD SS5.6)
- ⬜ 입금 일괄 처리 한 화면 최소 20행 (AC-4.9-6)
- ⬜ carry-over F1~F7 전수 해소
- ⬜ `cargo test` 전체 통과 + `cargo clippy -- -D warnings` clean
...
Sprint 착수 전 아래 항목에 대한 사용자 결정이 필요합니다.

### 1. PI-10: 마감 후 수정 사유 UX
- **옵션 A**: 모달 다이얼로그 (수정 버튼 클릭 시 팝업, 사유 입력 후 저장)
- **옵션 B**: 인라인 입력 (금액 수정 필드 옆에 사유 입력란 노출)
- **권장**: 옵션 A (모달) -- 사유 입력의 "의도적 행위" 느낌 강조, 실수 방지

### 2. PI-11: 마감 해제(reopen) 기능
- **옵션 A**: 마감 해제 불가 -- 마감 후에는 개별 건 수정만 (사유 입력 필수)
- **옵션 B**: 월 전체 마감 해제 가능 -- 전체를 confirmed 상태로 되돌림 (사유 입력 필수)
- **권장**: 옵션 A -- PRD에 마감 해제 언급 없음, 단순화 우선

### 3. PI-12: 수납 테이블 분리
- **옵션 A**: 별도 `payments` 테이블 (bills 1:1 관계)
- **옵션 B**: bills 테이블에 수납 컬럼 내장
- **권장**: 옵션 A (별도 테이블) -- 향후 분할 납부/환불 확장 여지, 감사 로그 분리 용이

### 4. 카드 계열 결제수단 식별 기준
- **옵션 A**: 결제수단 코드 name에 "카드" 문자열 포함 여부로 판별
- **옵션 B**: `payment_methods` 테이블에 `is_card_type` BOOLEAN 컬럼 추가 (V109 마이그레이션에 포함)
- **권장**: 옵션 B -- 사용자 추가 결제수단에도 정확한 판별 보장

### 5. carry-over F4 (N+1 쿼리) 범위
- `calendar.rs` N+1 쿼리만 수정 vs `attendance.rs` N+1(A54)도 함께 수정
- **권장**: `calendar.rs`만 수정 (F4) -- `attendance.rs` N+1은 PRD 성능 기준 충족 중이므로 별도 backlog 유지
