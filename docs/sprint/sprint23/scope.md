...
| 파일 | 수정 횟수 | 비고 |
|------|---------|------|
| src-tauri/src/commands/db.rs | [1회] | T1 after_connect 훅 ✅, T2 create_if_missing 가드, T6 유휴 close/재연결 |
| src-tauri/src/commands/integrity.rs | [0회] | T2 빈 DB fail-hard, T3 WAL 처리·다계층 폴백·소스 검증 |
| src-tauri/src/commands/backup.rs | [0회] | T4 소스 검증, 마지막 백업 축출 방지 |
| src-tauri/src/commands/paths.rs | [0회] | T5 config 처리 통일 (무음 fallback 제거, salt.bin SSOT) |
| src-tauri/src/commands/setup.rs | [0회] | T5 config 손상 처리 통일 |
