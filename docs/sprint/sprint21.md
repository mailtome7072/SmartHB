...
- ⬜ MoveAttendanceDialog가 교습기간 날짜 범위 기준으로 이동 대상 표시 (T3)
- ⬜ **단일월 교습기간 회귀 없음** — 기존과 동일한 그리드 표시 + 출결 토글 + 보강 등록 정상 (T4 a)
- ⬜ **다월 교습기간** — 달력월 밖 모든 날짜 표시 + 출결 토글 + 보강 등록 정상 (T4 b)
- ⬜ DEPLOY.md에 인쇄 미리보기 확인 항목 추가 (T0, A122)
- ⬜ `cargo test` 전체 통과 (441건 + T1 신규)
