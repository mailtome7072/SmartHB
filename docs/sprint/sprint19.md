...
- ⬜ `BillingGrid.tsx` 헤더 클릭 정렬 신규 구현 — T1 공통 훅 적용
  - 정렬 가능 컬럼: 이름, 학년, 청구액, 수납상태 등
  - 기본 정렬: 학년별+이름 가나다순
- ⬜ 스크롤 접근성 점검 — sticky header 미적용 상태이므로 `thead sticky top-0 z-10 bg-white` 추가
  - T2와 동일한 단일 스크롤 컨테이너 패턴 적용
