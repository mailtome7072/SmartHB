...
- ⬜ `attendance.rs` 신규 IPC `move_attendance(student_id, from_date, to_date)`:
  ...
  - 단위 테스트: 정상 이동, 비수업일로 이동, 월경계 차단, to_date 충돌 차단, present 외 상태 거부
- ⬜ 출결표 UI: 셀 컨텍스트 액션에 "수업일 이동" → 날짜 선택 다이얼로그. 이동된 셀은 `note` 툴팁 표시. TanStack Query 무효화 + 1단계 Undo(PRD §5.7 위험행위 확인)

**케이스 2 — 특정일 이후 영구 변경 (시계열 + 미처리만 재생성)**
...
- ⬜ 신규 IPC `apply_schedule_change(student_id, effective_date)`:
  - 변경일 D 이후 ~ 확정 교습기간 말까지, 해당 원생 `regular_attendances` 중 `status='present'` 행만 DELETE
  - 날짜 인식 스케줄로 D 이후 재생성 (`INSERT OR IGNORE`, off_date·입퇴교 범위 동일 적용)
  - 결석/보강완료/소멸/메모(`note` 존재) 행은 **보존** → 보존 건수 + 옛 요일 잔존 목록 반환
  - 미확정 교습기간 월은 건너뜀 (향후 `generate` 시 날짜 인식으로 자동 반영)
  - `audit::ScheduleChangedWithRegen` variant
  - 단위 테스트: present 재생성, 결석 보존, 보강완료 보존, 변경일 이전 불변, 미확정월 skip, 주당시간 변동 감지
- ⬜ 스케줄 변경 UI(`ScheduleEditor`): "이 날짜 이후 적용" 날짜 입력 → `set_schedule(effective_from=D)` + `apply_schedule_change(D)` 연계. 확인 다이얼로그("변경일 이후 출결을 재생성합니다. 결석/보강 N건은 보존됩니다.")
...
- ⬜ 원생 등록/수정/조회 전체 흐름
- ⬜ 출결 생성 + 출결표 + 토글
- ⬜ **수업일 변경 케이스1**: 특정일 1회성 이동 → 원래 날 비고 + 새 날 출석 + 메모, 청구 불변
...
### 신규 의존성 (예상)
- T0(수업일 변경): **신규 의존성 없음** (기존 `chrono` + `sqlx` 활용). 마이그레이션 V306 1건 추가
