...
- ⬜ 출결표 UI (조작 방식 = 클릭 → 다이얼로그, PI-26):
  ...
  - 도착일은 **다이얼로그 내 달력에서 선택** — OFF일/공휴일/기존 출결 있는 날/타월(他月)은 **비활성 + 사유 표시**(PI-25 차단 규칙을 UI 단에서 선반영)
...
| PI-26 | 케이스1 이동 UI 조작 방식 | **클릭 → 다이얼로그** — 옮길 출결이 있는(present) 셀 클릭 → "수업일 이동" → 다이얼로그 달력에서 도착일 선택. 드래그앤드롭 아님. 빈 셀 클릭은 보강 등록과 구분 (T0) | 2026-06-08 |
