...
**수정 방향** (정확한 레이아웃은 frontend-design 스킬 + 시각 검증으로 결정):
...
- ⬜ 삭제 시 감사 로그(`BillDeleted`) 기록 확인
- ⬜ 청구 화면에서 삭제 버튼 + 확인 다이얼로그 정상 동작
- ⬜ 실 DB 보정 절차 문서 완성
- ⬜ 퇴교 취소 → 월별 데이터 재생성 운영 워크플로우 안내 문서 완성 (T5-b)
- ⬜ 교습일정 인쇄가 3개 월 이상 걸침 교습기간에서도 각 월 달력이 읽을 수 있는 크기로 출력됨 (T6)
- ⬜ 교습일정 인쇄 1~2개월 걸침 기존 레이아웃 회귀 없음 확인 (T6)
...
- ⬜ (버그 B 처리 시) 다월 교습기간의 달력월 밖 출결(7/30·31, 9/1·2)이 그리드에 표시됨 (T7 버그 B) — 분리 시 후속 스프린트로 이월 명시
- ⬜ `cargo test` 전체 통과 (신규 테스트 포함)
