...
|---------|-----------|-------------------|
| A126 | MoveAttendanceDialog `yearMonth` prop -> `invalidationYm` 명확화 | T8에서 출결 프론트엔드 수정 시 함께 처리 (attendance 관련 스프린트) |
| A114 | sync_single_date 이력 패턴 통일 (4스프린트 이연) | attendance.rs 수정 스프린트이므로 T6에서 함께 처리 |
...
### T6: 백필 결과 안내 UI (프론트엔드)

**목표**: 마이그레이션 후 앱 시작 시 백필 결과를 사용자에게 안내.

**구현**:
- 백엔드 IPC: `get_backfill_result` -- 보정 건수/상세 조회 (app_settings 또는 별도 테이블에 기록)
- 프론트엔드: 앱 시작 시 토스트 또는 다이얼로그로 "N건의 보강 데이터가 보정되었습니다" 안내
- 1회 안내 후 다시 표시하지 않음
