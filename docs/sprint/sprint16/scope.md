...
## 시각검증 + 원생 폼 개선 (Session #11, 2026-06-11) — 사용자 검수 중 추가 요청
P0/P1/P2 반영분 실앱 시각검증(사용자) 진행 중 발견·요청된 원생 폼 UX 개선. 모두 구현·검수 완료(2026-06-11).
- **임시저장 실효화**: 3분 interval → **입력 즉시 저장**(이탈 전 손실 방지). dirty 시 "자동 임시저장됨" 안내. 입력 후 이탈(이동 경고에서 '이동')해도 재진입 시 '이어서 작성' 배너로 복원.
- **그리드 임시저장 표시**: 원생 목록 이름 칸 `✎ 임시저장` 배지(수정 draft 보유 원생) + `신규 등록` 버튼 `✎ 작성 중` 배지(신규 draft 보유). 목록 진입·창 포커스 복귀 시 재계산. `STUDENT_DRAFT_PREFIX` export로 키 공유.
- **성별·학년·학교 필수화**: 신규 등록 시 기본값 제거 → `(미지정)`으로 시작(학교 패턴). 저장 시 미지정이면 필수 입력 검증으로 차단+입력 유도. 라벨 `*` 표기.
- **학교급↔학교명 정합성**: 초등인데 학교명에 '중학교' / 중등인데 '초등학교' 포함 시 확인 유도 검증.
- **검증/저장 실패 메시지 팝업화**: 인라인 빨간 텍스트 → 공용 `ErrorDialog` 모달.
> 검증: tsc / lint 통과(프론트 전용). 사용자 실앱 검수 완료.
> 메모: 필수 검증은 신규·수정 폼 공통 적용(수정 모드는 기존 값 유지라 보통 자동 통과). school_id 는 DB nullable 유지 — 폼 레벨 강제만.
