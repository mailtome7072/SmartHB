...
## 추가 반영 — 수동검증 5건 발견 후 회고 보완 (2026-07-29)

### 초기 감사가 놓친 것: billing.rs "SAFE" 전제의 한계

초기 sprint-review에서 billing.rs를 SAFE로 판정한 근거는 "커밋 diff에 billing.rs 변경 없음, 청구는 study_periods.year_month 기준 집계"였다. 그러나 수동검증에서 발견된 버그(5f65b03)는 billing.rs 내부의 스케줄 이력 처리가 잘못됐음을 보여줬다: `effective_to IS NULL`(현행 스케줄만)로 산정하는 로직이 "교습기간이 스케줄 변경 적용일보다 앞선 경우"를 처리하지 못했다.

이 케이스는 다월 교습기간(경계일이 달력월과 다름)과 스케줄 이력 변경이 동시에 존재할 때만 발생한다. 초기 감사는 두 조건 중 하나(다월 교습기간)에는 집중했으나, 두 조건의 교집합(다월 + 변경 이력)을 커버하는 테스트가 billing.rs에 없었다. 자동 검증이 PASS했어도 교집합 케이스는 테스트 공백이었다.

**재발 방지**: billing.rs 같은 핵심 집계 함수를 리뷰할 때, "파일 변경 없음"이 "버그 없음"을 보장하지 않는다. 관련 의존성(스케줄 이력, 교습기간 경계)이 변경된 스프린트에서는 집계 함수의 교집합 케이스도 점검 대상으로 포함해야 한다.

### 수동검증에서 5건 추가 발견: 다월이 청구·이동·재설정 전반에 영향

Sprint 24 초기 구현은 출결 태깅·표시·이동 판정의 다월 버그를 9건 수정했다. 그러나 수동검증에서 추가로 발견된 5건은 다월 교습기간의 영향 범위가 더 넓었음을 보여준다:

1. **청구 이력 인식 누락** — billing.rs가 현행 스케줄만 참조하는 구조적 문제
2. **교습기간 재설정 후 재태깅 갭** — V313(1회성)이 "이후 변경"을 커버하지 못한 설계 갭
3. **이동 팝업 달력 범위 부족** — 백엔드 B1 변경에 프론트엔드가 따라가지 못한 레이어 갭
4. **청구 재확인 UX 부재** — 시수 변경이 기존 청구에 영향을 주는데 알림 경로가 없었음
5. **코드 리뷰 지적 4건 반영** — M1(N+1), M2(이중부정), L1(모듈 결합), L2(관찰 불가 로그)

이 중 1~4는 수동검증 중 실환경 사용에서 발견된 것으로, 자동 테스트만으로는 발견이 어려운 사용자 시나리오 기반 결함이었다. 단일 개발자 + 실사용 병행 검증이 효과적인 QA 방법론임을 재확인.

### startup 재동기화로 "자가 치유" 경로 확립

V313(일회성) + reconcile 런타임 판(지속) + startup 1회 실행의 3단계 방어가 구축됐다. 앱이 시작될 때마다 year_month 불일치를 자가 치유하는 경로가 있으므로, 향후 교습기간 관련 버그가 발생해도 "앱 재시작"으로 1차 복구가 가능하다. 단, eprintln!으로 로그를 남기므로 프로덕션 관찰은 불가(A132 재확인).

### 재리뷰 액션 아이템 추가

| ID | 항목 | 우선순위 | 대상 위치 | 기한 | 상태 |
|----|------|----------|-----------|------|------|
| A135 | billing.rs 핵심 집계 함수에 "다월 교습기간 + 스케줄 이력 변경" 교집합 케이스 테스트 추가 — generate_bills / get_billing_summary 의 이력 인식 수정 검증 테스트(5f65b03 추가분)를 baseline으로 확장 | Medium | `src-tauri/src/commands/billing.rs` | 다음 billing.rs 수정 스프린트 | 📋 예정 |
| A136 | 스케줄 삭제 warnAffectedBills 커트라인을 삭제 스케줄 effective_from 기준으로 교체 검토 — 현행 today 기준은 오늘 이전 종료 교습기간 청구를 팝업 미표시 (L4, 팝업 advisory이므로 긴급하지 않음) | Low | `src/components/students/schedule-editor.tsx` — remove onSuccess | 다음 스케줄 편집기 수정 스프린트 | 📋 예정 |
