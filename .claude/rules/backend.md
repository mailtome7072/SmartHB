...
- 파일명 형식: `{NNN}__{설명}.sql` — `NNN` 은 **3자리 zero-pad** 숫자 (sqlx 는 숫자 버전 prefix 만 파싱; 문서에서 "V305" 등 V 표기는 통칭)
  - 3자리는 100+ 확장 시 사전순 정렬을 보장 (`sqlx::migrate!` 가 파일명 사전순으로 적용)
  - 예: `001__create_code_tables.sql`, `101__create_students_and_schedules.sql`
  - **번호 정책 (실제 운용)**: 기능 영역별 100단위 블록을 끊는다.
    - `001~099` — 코드 테이블·감사 로그·앱 설정 (인프라성)
    - `101~199` — 핵심 도메인 (원생/스케줄/교습기간/수업료/학교)
    - `200~299` — 시드 데이터
    - `301~` — 보정/확장. 새 도메인 영역 추가 시 다음 100단위에서 시작
- `sqlx migrate run` 으로 적용

### SQLx 쿼리
> **2026-06 개정 (코드리뷰 P1-10, 사용자 결정)**: 본 프로젝트는 **런타임 `sqlx::query()` + bind 파라미터** 방식을 표준으로 한다. `query!`/`query_as!` 매크로(컴파일 타임 검사)는 채택하지 않음 — `.sqlx/` 오프라인 캐시도 운용하지 않는다. 대신 스키마-쿼리 정합은 **인메모리 마이그레이션 단위 테스트로 필수 커버**한다.
- 사용자 입력은 반드시 `bind()` 파라미터로 전달 — raw query string 직접 연결(concat) 금지 (SQL 인젝션 방지)
- 동적 SQL 은 정적 fragment 결합만 허용, `format!` 보간은 컴파일 타임 고정 allowlist 상수만
- 새 쿼리를 추가하면 해당 `_impl` 함수를 `db::test_pool_in_memory()` 기반 단위 테스트로 커버 (스키마 변경 회귀 검출)
...
### 청구 상태 워크플로우 (PRD §4.9.7 — V111 개정)
> **2026-06 개정 (코드리뷰 P1-10)**: '마감(closed)' 개념은 **V111 마이그레이션에서 폐기**됨 (`111__remove_billing_close_concept.sql`). 1인 운영 실사용 패턴에 맞춰 2단계로 축소 — 마감 개념을 재도입하지 말 것.
- 청구 데이터는 **2단계 상태**로 관리: `draft`(미확정) → `confirmed`(확정)
- 상태 전이 규칙
  - 신규 생성: `draft` (월 중 입퇴교 포함)
  - `confirmed`: 원장 검토 완료 후, **확정 청구 수정 시 확인 다이얼로그 필수**
- 스키마: `bills.status` TEXT(draft/confirmed) + `confirmed_at`
- 상태 전이 규칙 위반은 비즈니스 규칙 단위 테스트로 보장 (PRD §6.5)
...
| 학습보고서 | `(분기, 원생)` UNIQUE |
| 청구 | `(원생, 청구년월)` UNIQUE |
| 학사 일정 | 중복불가 코드는 `(일자, 코드)` 중복 금지 — **앱 레벨(IPC) 강제** (`is_duplicate_blocked` 가 코드 속성이라 단순 UNIQUE 표현 불가) |
| 교습기간 | 일자 중첩 금지 — **앱 레벨(IPC) 강제** (행 간 제약은 SQLite CHECK 표현 불가, academic.rs 검증 + 단위 테스트) |
...
- **High**: `unwrap()` 남용 (panic 유발), 마이그레이션 없는 스키마 변경, 새 쿼리의 단위 테스트 누락, `PRAGMA integrity_check` 누락, 락 파일 heartbeat 미구현, **PRD §6.2 UNIQUE 제약 누락** (특히 청구 `(원생, 청구년월)`)
- **Medium**: `app_data_dir()` 사용 (클라우드 동기화 폴더 정책 위반), 백업 로직이 트랜잭션 외부 실행, **확정(confirmed) 청구 수정 시 확인 다이얼로그 미강제** (V111 2단계 모델)
