...
description: "⚠️ v1.4.0 배포(Sprint 22) 후 2026-07-22 학원PC 데이터 전면소실 사고 → 복구 완료. Sprint 23(재발방지 A+B, ADR-012 A안) 계획·커밋·push 완료, 미구현. 새 세션 진입 시 가장 먼저 확인"
metadata:
  node_type: memory
  type: project
...
## ⚠️ 2026-07-22 프로덕션 데이터 소실 사고 + 복구 + Sprint 23 계획
...
- ⬜ **2026-07-23 학원 PC에서 MYBOX 동기화 후 앱 실행(PIN 원장확인) → 원생 31명 최종 확인 예정.**

### Sprint 23 계획 (커밋·push 완료 `521c3ef`, 미구현)
- 주제: 사고 재발방지 A(데이터안전) + B(2번째PC로그인). **DB 마이그레이션·새 의존성 없음.**
- **ADR-012 = A안**(`docs/arch/adr-012-db-live-location.md`): 라이브 DB를 **클라우드 폴더에 유지 + 접근 강화**(데이터 로컬 이전 안 함). 매트릭스 A 4.30 > B 3.55 > C 3.30. B안(로컬+핸드오프)은 ROADMAP에 향후 Phase 후보 등록(승격 트리거: A 배포 후 클라우드 간섭 재관찰 시).
...
### ⬜ 다음 세션 진입 시
1. **학원 PC 데이터 복구 최종 확인**(2026-07-23 이후)
2. 준비되면 **`/sprint-dev 23`** 구현 진입 (T0 완료, T1부터). ⚠️ `/sprint-dev`는 사용자가 직접 입력.
