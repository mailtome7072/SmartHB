...
description: "v1.5.1 프로덕션 사고(2026-08-01) — V313 pending 첫 실행 시 migrator.run 무한 블록(Windows 실환경). 근본원인 미해결, V314 때 재발. 우회=DB에 마이그레이션 선적용"
metadata: 
  node_type: memory
  type: project
  originSessionId: 2de8b3ba-b8a7-4d3d-bec8-87adf8a9081b
  modified: 2026-08-01T11:42:15.634Z
---
...
- **Mac 독립 harness로는 재현 불가**: 평문 sqlx 마이그레이터 / rusqlite 백업+동시연결+쓰기트랜잭션 / **sqlx+SQLCipher(libsqlite3-sys 공유) 마이그레이터** / 전체 interleaving 전부 <20ms 정상. → **실 Windows Tauri 런타임 고유의 무한 블록**으로 추정(파일락/AV/스레드풀 등 미확정).

**Why**: 근본원인 미해결. 증상만 우회했으므로 **다음 DB 마이그레이션(V314) 배포 시 동일 재발**한다.

**How to apply**:
- 우회책: 앱이 못 하는 마이그레이션을 오프라인으로 선적용 → pending 제거 → v1.5.1이 구간 건너뜀. 절차=[[offline-migration-apply]]. 2026-08-01 프로덕션 DB에 V313 선적용+재암호화+교체로 양 PC 정상화(원생33/출결978 무손실, 검사8 오태깅 0건, CS 7/30 표시 정상).
- **v1.5.2 필수**: db::initialize의 backup↔migrate 구간 견고화 (최소 timeout+fail-soft로 무한 hang 방지). 계획: `docs/sprint/sprint25.md`.
