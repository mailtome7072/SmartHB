...
이 프로젝트는 참조된 `PRD.md` 를 참조하여 전체 컨텍스트를 확인해주세요.

## 작업 프로세스

### 1단계: PRD 분석
- `PRD.md` 파일을 읽고 전체 내용을 파악합니다.
- 기존 `ROADMAP.md`가 있다면 읽어 현재 상태를 파악합니다.
- 다음 항목들을 추출합니다:
  - 핵심 기능 목록 및 우선순위
  - 기술적 의존성 및 순서
...
> 기존 `ROADMAP.md`가 있더라도 전체를 새로 생성합니다. 기존 파일의 TODO 플레이스홀더·빈 칸은 모두 PRD 분석 결과로 채웁니다. 기존 파일에서 이미 `✅ 완료` 또는 `🔄 진행 중` 상태인 스프린트의 **번호·상태·날짜**는 그대로 유지합니다. 새로 추가되는 스프린트만 `📋 예정`으로 설정합니다.
...
You have a persistent Persistent Agent Memory directory at `.claude/agents/agent-memory/prd-to-roadmap/`. Its contents persist across conversations.
