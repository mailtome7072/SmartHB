...
- [Sprint 현황](sprint-status.md) -- Sprint 12 계획 수립 완료, Phase 4 마지막 sprint (공지문 이미지)
- [마이그레이션 번호 정책](migration-numbering.md) -- V111 완료 (Sprint 12는 DB 마이그레이션 없음 — 파일 시스템 + app_settings 활용)
...
- 마지막 완료 스프린트: Sprint 11 (2026-05-29)
- 다음 스프린트 번호: 12 (계획 수립 완료)
...
- **Phase 구조**: 7 Phase + 1.5(안정화) + 1.5b(추가 안정화). Phase 1~3 완료, Phase 4 진행 중 (Sprint 11~12)
- **핵심 참조 문서**: PRD.md (v1.5.1), ROADMAP.md (SSOT), docs/phase/phase1.md (Phase 설계)
- **데이터 모델**: docs/data-model.md v1.5 (V001~V008 마이그레이션 가이드)

## Sprint 12 주요 설계 결정 사항

- 이미지 생성: `html-to-image` (frontend.md 명시, ADR 불필요)
- 텍스트박스 드래그: PI-14 `react-rnd` 권장 (사용자 확인 대기)
- 배경서식 저장: 파일 시스템(`smarthb/assets/`) — DB 테이블 미사용
- 레이아웃 저장: `app_settings` 테이블 JSON (key='notice_layout')
- 공지문 PNG 경로: `{data_root}/output/{YYYYMM}/{YYYYMM}_{원생이름}.png`
- DB 마이그레이션: 없음 (V111이 최신, app_settings 기존 테이블 활용)
- CSV 가져오기: Sprint 15로 이연 (Sprint 12 범위 제외)
- 마감(closed) 개념 폐기됨 — 공지문은 confirmed 상태 청구만 대상
...
- PI-10 (Sprint 11): 마감 후 수정 사유 UX — T5 진입 시 사용자 확인 (모달 vs 인라인)
- PI-11 (Sprint 11): 마감 해제(reopen) 기능 필요 여부 — T5 진입 시 사용자 확인
- PI-12 (Sprint 11): 수납 테이블 분리 여부 — T1 설계 시 결정
