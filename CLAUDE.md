...
### ESLint 설정
- 현재 ESLint 설정 파일은 별도 존재하지 않음 — `eslint-config-next` 기본 규칙만 적용된다 (`next lint` 실행 시 자동 로드).
- ESLint 9 + Next.js 15 조합에서 커스텀 규칙을 추가할 때는 **flat config(`eslint.config.mjs`)**를 사용한다. 레거시 `.eslintrc.*`는 신규 작성 금지.
- flat config 도입 후 `next lint`가 실패한다면 `eslint-config-next` flat 지원 버전 사용 여부를 먼저 확인.
...
새 플러그인 추가 시 (예: `tauri-plugin-fs`, `tauri-plugin-dialog`):
